...
class SendScreen(CScreen):
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a FairCoin address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid FairCoin Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share FairCoin Request"))
